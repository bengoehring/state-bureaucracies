# This file merges the MS correctional officer data with the MS voter file. 
...
fastlink_input <- readRDS("input-data/ms-fastlink-input.rds")
source("code/merge-fun.R")

#fastlink_input <- readRDS("../data/correctional-officers/merge-inputs/ms-fastlink-input.rds")
...
ms_correctional_join <- fastlink_input$ms_correctional_join

# blocking objects ####
block_obj_25 <- blockData(ms_correctional_join, 
                          voters_within_25km, 
                          varnames = c("facility"))
block_obj_50 <- blockData(ms_correctional_join, 
                          voters_within_50km, 
                          varnames = c("facility"))
block_obj_100 <- blockData(ms_correctional_join, 
                           voters_within_100km, 
                           varnames = c("facility"))

# merges with 25km voter set ####
out_25_.9 <- map(names(block_obj_25), 
                 ~merge_by_block(ms_correctional_join,
                                 voters_within_25km,
...
fastlink_output <- list("out_25_.9" = list("fl_obj" = out_25_.9,
                        ...
                        "input_correctional_data" = ms_correctional_join)
saveRDS(fastlink_output, 
        "output-data/ms-fastlink-output.rds")
